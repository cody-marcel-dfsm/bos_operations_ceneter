...
- **Status:** Superseded for live-route certification by the Education Center
  identity migration. The concurrency measurements remain historical evidence.
...
- Protocol initialization and live tool discovery still complete before any
  operational tool call: `scripts/smoke-education-center-director-query.mjs:271` and
  `scripts/smoke-education-center-director-query.mjs:311`.
- The enrollment query starts only after the complete required live tool
  ...
  `scripts/smoke-education-center-director-query.mjs:339`.
- The authenticated context and enrollment reads execute concurrently and are
  ...
  `scripts/smoke-education-center-director-query.mjs:352`.
- Existing fail-closed context, tool-catalog, enrollment-shape, and camp-field
  checks remain active: `scripts/smoke-education-center-director-query.mjs:362`.
- Progress output contains only fixed phase/state labels and HTTP status; the
  ...
  `scripts/smoke-education-center-director-query.mjs:430`.
- The focused regression test proves discovery precedes both read calls, the
  ...
  `tests/education-center-director-live-smoke.test.mjs:123`.
...
- The recorded live scoped query predates the current Education Center route
  and does not certify its deployed runtime.
- `node --test tests/education-center-director-live-smoke.test.mjs`: passed, 16 tests.
