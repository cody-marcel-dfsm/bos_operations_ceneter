...
- Subservice plugins contribute skills, product metadata, and server
  ...
  connection.
- ChatGPT/Codex binds the root BOS registered app by its durable `asdk_app_*`
  identity. Installation-scoped `plugin_asdk_app_*` connector wrappers are
  generated by the host and never appear in product manifests or `.app.json`.
