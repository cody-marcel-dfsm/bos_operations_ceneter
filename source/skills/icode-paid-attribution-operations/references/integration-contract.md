...
Search Gmail for a bounded date range using these known selected iCode organization patterns:
...
selected iCode organization has successfully executed the BOS
`icode_create_offline_ad_conversions` mutation. Treat that provider-confirmed
...
`icode-offline-ad-conversions.conversions.create`. The legacy capability
`icode-offline-ad-conversions.action.sync` may appear during a rolling
...
## iCode offline conversion MCP action

When the live `bos_icode` manifest publishes
`icode_create_offline_ad_conversions`, prefer it for the complete bounded
Gmail-to-Google-Ads workflow. The client instruction can be:

> Sync iCode offline ad conversions from 2026-07-01 through 2026-07-21.
