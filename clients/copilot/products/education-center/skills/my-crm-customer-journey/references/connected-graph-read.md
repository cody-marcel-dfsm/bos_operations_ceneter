...
The active product MCP resources/list response may advertise an
application-owned graph on its existing authenticated connection. In Education
Center, inspect Education Center MCP; in My CRM, inspect My CRM MCP. Select the
resource by its declared app, contract and exact selected context metadata; never build a URI from an identifier. Read the
