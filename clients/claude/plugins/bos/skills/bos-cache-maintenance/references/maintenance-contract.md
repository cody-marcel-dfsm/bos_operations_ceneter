...
Journey authoring resources use the same OS-user cache root through the packaged
journey-contract adapter. Its invalidation scopes are `resource`, `service`,
`application`, and `current_authority`. Every request carries the exact current
server-derived authority input. Diagnostic output contains digests and timing,
without raw authority values or descriptor payloads.
