...
import {
  ICODE_DIRECTOR_REQUIRED_TOOLS,
  IcodeDirectorSmokeFailure,
  currentLocalWeek,
  runIcodeDirectorSmoke
} from "../scripts/smoke-icode-director-query.mjs";
...
test("director live smoke executes context and a bounded camp-roster query", async () => {
  ...
  const fetchImpl = async (_url, options) => {
    ...
    if (body.method === "tools/list") {
      return response({
        ...
        id: body.id,
        result: {
          tools: ICODE_DIRECTOR_REQUIRED_TOOLS.map((name) => ({ name }))
        }
      });
    }
  ...
  const enrollmentCall = requests.find(
    (request) => request.body.params?.name === "icode_list_enrollments"
  );
...
test("director live smoke prints a secret-free server prompt for missing tools", async () => {
  ...
  await assert.rejects(
    ...
    (error) => {
      ...
      assert.match(error.message, /icode_list_enrollments/);
      assert.match(error.message, /icode_search_students/);
...
test("protocol mismatch fails before tool discovery", async () => {
  ...
  await assert.rejects(runIcodeDirectorSmoke({
    apiKey: "test-secret",
    fetchImpl,
    timeZone: "America/Denver"
