...
test("Codex OAuth installation rejects an obsolete app file beside the MCP binding", async () => {
  const home = await temporaryHome();
  ...
  await writeFile(join(target, ".app.json"), JSON.stringify({
    apps: { bos: { id: "obsolete", required: true } }
  }));
...
test("Codex OAuth installation rejects a mismatched BOS MCP resource", async () => {
  const home = await temporaryHome();
...
test("Codex OAuth installation rejects a malformed MCP transport", async () => {
  const home = await temporaryHome();
  await applyInstallationRaw({ home, product: "bos" });
  const mcpPath = join(installedProduct(home, "bos"), ".mcp.json");
  const mcp = JSON.parse(await readFile(mcpPath, "utf8"));
...
test("unmanaged package-owned MCP package is adopted", async () => {
  const home = await temporaryHome();
  const desired = join(root, "clients", "codex", "plugins", "bos");
  const target = installedProduct(home, "bos");
  await mkdir(join(codexMarketplaceRoot(home), "plugins"), { recursive: true });
...
test("unmanaged obsolete app package converges to package-owned MCP", async () => {
  const home = await temporaryHome();
  const desired = join(root, "clients", "codex", "plugins", "bos");
  const target = installedProduct(home, "bos");
  await mkdir(join(codexMarketplaceRoot(home), "plugins"), { recursive: true });
  ...
  await writeFile(join(target, ".app.json"), JSON.stringify({
    apps: { bos: { id: "plugin_asdk_app_obsolete", required: true } }
  }));
