...
async function isObsoleteCodexAppConfig(path, expectedName) {
  try {
    const config = await readJson(path);
    const entries = Object.entries(config?.apps ?? {});
    if (entries.length !== 1) return false;
    const [name, app] = entries[0];
    return name === expectedName && typeof app?.id === "string";
  } catch {
    return false;
  }
}
...
async function inspectTarget(paths, desired) {
  ...
  if (
    ...
    if (await isObsoleteCodexAppConfig(join(paths.target, ".app.json"), desiredMetadata.name)) {
